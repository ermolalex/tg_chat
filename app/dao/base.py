...
class BaseDAO(Generic[T]):
    ...
    async def find_one_or_none_by_id(cls, data_id: int, session: AsyncSession):
        # Найти запись по ID
        logging.info(f"Поиск {cls.model.__name__} с ID: {data_id}")
        try:
            query = select(cls.model).filter_by(id=data_id)
            result = await session.execute(query)
            record = result.scalar_one_or_none()
            if record:
                logging.info(f"Запись с ID {data_id} найдена.")
            else:
                logging.info(f"Запись с ID {data_id} не найдена.")
            return record
        except SQLAlchemyError as e:
            logging.error(f"Ошибка при поиске записи с ID {data_id}: {e}")
            raise
    ...
    async def find_all(cls, session: AsyncSession, filters: BaseModel):
        ...
        filter_dict = filters.model_dump(exclude_unset=True)
        logging.info(f"Поиск всех записей {cls.model.__name__} по фильтрам: {filter_dict}")
        try:
            query = select(cls.model).filter_by(**filter_dict)
            result = await session.execute(query)
    ...
    async def add_many(cls, session: AsyncSession, instances: List[BaseModel]):
        ...
        logging.info(f"Добавление нескольких записей {cls.model.__name__}. Количество: {len(values_list)}")
        new_instances = [cls.model(**values) for values in values_list]
        session.add_all(new_instances)
        try:
            await session.commit()
            logging.info(f"Успешно добавлено {len(new_instances)} записей.")
        except SQLAlchemyError as e:
            await session.rollback()
            logging.error(f"Ошибка при добавлении нескольких записей: {e}")
            raise e
    ...
    async def update(cls, session: AsyncSession, filters: BaseModel, values: BaseModel):
        ...
        filter_dict = filters.model_dump(exclude_unset=True)
        values_dict = values.model_dump(exclude_unset=True)
        logging.info(f"Обновление записей {cls.model.__name__} по фильтру: {filter_dict} с параметрами: {values_dict}")
        query = (
        ...
        try:
            result = await session.execute(query)
            await session.commit()
            logging.info(f"Обновлено {result.rowcount} записей.")
            return result.rowcount
        except SQLAlchemyError as e:
            await session.rollback()
            logging.error(f"Ошибка при обновлении записей: {e}")
            raise e
    ...
    async def delete(cls, session: AsyncSession, filters: BaseModel):
        ...
        filter_dict = filters.model_dump(exclude_unset=True)
        logging.info(f"Удаление записей {cls.model.__name__} по фильтру: {filter_dict}")
        if not filter_dict:
            logging.error("Нужен хотя бы один фильтр для удаления.")
        ...
        try:
            result = await session.execute(query)
            await session.commit()
            logging.info(f"Удалено {result.rowcount} записей.")
            return result.rowcount
        except SQLAlchemyError as e:
            await session.rollback()
    ...
    async def count(cls, session: AsyncSession, filters: BaseModel):
        ...
        filter_dict = filters.model_dump(exclude_unset=True)
        logging.info(f"Подсчет количества записей {cls.model.__name__} по фильтру: {filter_dict}")
        try:
            query = select(func.count(cls.model.id)).filter_by(**filter_dict)
            result = await session.execute(query)
            count = result.scalar()
            logging.info(f"Найдено {count} записей.")
            return count
        except SQLAlchemyError as e:
            logging.error(f"Ошибка при подсчете записей: {e}")
            raise
    ...
    async def paginate(cls, session: AsyncSession, page: int = 1, page_size: int = 10, filters: BaseModel = None):
        ...
        try:
            ...
            records = result.scalars().all()
            logging.info(f"Найдено {len(records)} записей на странице {page}.")
            return records
        except SQLAlchemyError as e:
            logging.error(f"Ошибка при пагинации записей: {e}")
            raise

    @classmethod
    async def find_by_ids(cls, session: AsyncSession, ids: List[int]) -> List[Any]:
        """Найти несколько записей по списку ID"""
        logging.info(f"Поиск записей {cls.model.__name__} по списку ID: {ids}")
        try:
            query = select(cls.model).filter(cls.model.id.in_(ids))
            result = await session.execute(query)
            records = result.scalars().all()
            logging.info(f"Найдено {len(records)} записей по списку ID.")
            return records
        except SQLAlchemyError as e:
            logging.error(f"Ошибка при поиске записей по списку ID: {e}")
            raise
    ...
    async def upsert(cls, session: AsyncSession, unique_fields: List[str], values: BaseModel):
        ...
        logging.info(f"Upsert для {cls.model.__name__}")
        try:
            ...
            if existing:
                ...
                await session.commit()
                logging.info(f"Обновлена существующая запись {cls.model.__name__}")
                return existing
            else:
                ...
                session.add(new_instance)
                await session.commit()
                logging.info(f"Создана новая запись {cls.model.__name__}")
                return new_instance
        except SQLAlchemyError as e:
            await session.rollback()
    ...
    async def bulk_update(cls, session: AsyncSession, records: List[BaseModel]) -> int:
        ...
        logging.info(f"Массовое обновление записей {cls.model.__name__}")
        try:
            updated_count = 0
            ...
            await session.rollback()
            logging.error(f"Ошибка при массовом обновлении: {e}")
            raise